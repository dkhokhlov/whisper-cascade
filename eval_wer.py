...
Three dataset sources are supported (dispatched by EVAL_DATASET):
  ...
  - sierra-research/mu-bench : NOT loaded via datasets (its builder forces the
                               torchcodec encoder, which this CPU project does
                               not depend on). The metadata.jsonl manifest and
                               the per-utterance wav files are downloaded
                               directly with huggingface_hub; wavs are 8 kHz
                               mono telephone speech and are resampled to
                               16 kHz. EVAL_CONFIG maps to a mu-bench locale
                               (en -> en-US, es -> es-MX, or a full locale
                               such as "en-US"). License CC-BY-NC-4.0: this
                               source is for transcription-quality testing
                               only; do not redistribute the audio or
                               transcripts.
...
Environment:
    ...
    EVAL_CONFIG      Dataset config / language code (default en_us). For
                     mu-bench this is the locale (en, es, or en-US / es-MX).
...
# Whisper expects 16 kHz mono float32. Sources that ship a different sample
# rate (mu-bench is 8 kHz telephone) are resampled to this.
TARGET_SR = 16000
...
# mu-bench EVAL_CONFIG shorthand -> locale. Accepts the shorthand (en, es) or
# a full locale (en-US); unknown values pass through unchanged.
_MUBENCH_LOCALE = {"en": "en-US", "es": "es-MX"}


def _iter_mubench(dataset, config, split):
    """Download mu-bench manifest + wavs directly (bypasses the datasets audio
    feature, which would require torchcodec). Wavs are 8 kHz mono; the caller
    resamples to 16 kHz. Reference field is "transcript"."""
    from huggingface_hub import hf_hub_download
    token = _hf_token()
    locale = _MUBENCH_LOCALE.get(config, config)
    meta = hf_hub_download(repo_id=dataset, repo_type="dataset",
                           filename="metadata.jsonl", token=token)
    with open(meta, encoding="utf-8") as fh:
        rows = [json.loads(line) for line in fh if line.strip()]
    for row in rows:
        if row.get("locale") != locale:
            continue
        ref = (row.get("transcript") or "").strip()
        fname = row.get("file_name") or row.get("audio_path")
        if not fname:
            yield ref, None, None
            continue
        wav_path = hf_hub_download(repo_id=dataset, repo_type="dataset",
                                   filename=fname, token=token)
        array, sr = sf.read(wav_path)
        if array.ndim > 1:
            array = array.mean(axis=1)
        yield ref, array.astype(np.float32), sr
...
def iter_samples(dataset, config, split):
    ...
    """
    if dataset == "sierra-research/mu-bench":
        return _iter_mubench(dataset, config, split)
...
def main() -> int:
    ...
    # mu-bench is CC-BY-NC-4.0: do not redistribute transcripts, so the
    # per-utterance ref/hyp samples are withheld from its result object. Only
    # aggregate metrics (wer, n, rtf) are reported for that source.
    is_mubench = EVAL_DATASET == "sierra-research/mu-bench"
    result = {
        "model": MODEL_ASR,
        "quant": QUANT or None,
