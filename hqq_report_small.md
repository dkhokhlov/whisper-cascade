...
4-bit grouped quantization. The model size shrank from 966.94 MB (fp32) to
294.92 MB (69.5% reduction). The Word Error Rate (WER) on 100 English samples
...
Across the 11 tested configs (5 `fleurs`, 4 `talkbank`, 2 `mu-bench`), small
fp32 beats base fp32 by 14-51% relative on 10 of 11 configs (the one exception
is `mu-bench` en-US, where small is 5% worse than base, within sample noise).
Hindi is still not usable at either precision. The cross-reference table is
below.
...
- [`sierra-research/mu-bench`](https://huggingface.co/datasets/sierra-research/mu-bench):
  ...
  this project does not depend on, so the `metadata.jsonl` manifest and the
...
| Model size on disk     | 966.94 MB     | 294.92 MB  | -672.02 MB       | -69.5%         |
| Weights only (qmodel)  | 966.94 MB     | 292.99 MB  | -673.95 MB       | -69.7%         |
...
- `whisper-small` English WER (0.0660) is lower than `whisper-base` (0.0985)
  ...
  the size cost is 967 MB fp32 / 295 MB HQQ vs 290 MB / 105 MB for base and
  151 MB / 62 MB for tiny.
...
|--------|--------|----------|---------|-------------|----------|-----|----------|---------|------------|-----------|
| en-US  | en     | 0.2783   | 0.2885  | +0.0102     | +3.7%    | 200 | 41.3 s   | 53.3 s  | 0.055      | 0.070     |
| es-MX  | es     | 0.4742   | 0.4606  | -0.0136     | -2.9%    | 200 | 43.7 s   | 67.6 s  | 0.051      | 0.078     |
...
- The WERs are high (0.25-0.49) because conversational telephone speech is
  ...
  beats `whisper-base` on every telephone config except `mu-bench` en-US (see
  the cross-reference).
...
| mu-bench | en-US | 0.2777    | 0.2994   | 0.2650    | 0.2662   | 0.2783     | 0.2885    | +0.0133           | +5.0%           |
| mu-bench | es-MX | 0.6884    | 0.6647   | 0.5707    | 0.5758   | 0.4742     | 0.4606    | -0.0965           | -16.9%          |
...
Notes:
- `whisper-small` beats `whisper-base` on 10 of 11 configs. The one exception
  is `mu-bench` en-US, where small is 5% worse than base (0.2783 vs 0.2650).
  This is within the n=200 sample noise and is the only config where the
  larger model does not improve on base.
- The HQQ size reduction grows with model size (59% / 64% / 70%): a larger
  ...
  embedding. Small is 2.8x the size of base at HQQ 4-bit (295 MB vs 105 MB).
...
ASR_DEVICE=cuda EVAL_DATASET=sierra-research/mu-bench EVAL_CONFIG=en EVAL_SPLIT=train EVAL_LIMIT=200 \
  QUANT=hqq MODEL_ASR=./whisper-small-hqq-4bit EVAL_OUT=small_mubench_en_hqq.json .venv-gpu/bin/python eval_wer.py
...
- Telephone: `eval_telephone/small_talkbank_<lang>_<fp32|hqq>.json` and
  `eval_telephone/small_mubench_<lang>_<fp32|hqq>.json` (mu-bench files hold
...
- Evaluated on `fleurs` (en, es, fr, de, hi), `talkbank_4_stt` (en, es, fr,
  ...
  measured.
- `whisper-small` is not strictly better than `whisper-base` on every config:
  it is 5% worse on `mu-bench` en-US (within sample noise) and the only
  non-monotonic case in the table.
