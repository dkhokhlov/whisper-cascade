...
97.22 MB, 33.0% smaller than the unquantized fp16 model
...
- **fp16 (default): `encoder_model.onnx` + `decoder_model_merged.onnx`** —
  ...
  slowdown is expected) but loads less RAM.
...
evidence JSONs are committed under `evals/multilingual/` (prefix `base_`) and
`evals/telephone/` (prefix `base_`) in
